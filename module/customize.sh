...
MODULE_CONFIG="/data/adb/.config/Velfox"
...
abort_unsupported_arch() {
	...
	ui_print "! This device architecture is not supported by Velfox Tweaks."
...
recognize_soc() {
	...
	*gs* | *Tensor* | *tensor*) SOC=5 ;;
	...
	5) ui_print "- Implementing tweaks for Tensor" ;;
	0) return 1 ;;
	esac
}
...
extract "$ZIPFILE" 'system/bin/velfox_utility' "$MODPATH"
extract "$ZIPFILE" velocity.banner.avif "$MODPATH"

# Extract Architecture Binaries
extract "$ZIPFILE" "libs/$ARCH_TMP/sec.velfoxruntime" "$TMPDIR"
extract "$ZIPFILE" "libs/$ARCH_TMP/velfox_profiler" "$TMPDIR"

cp "$TMPDIR/libs/$ARCH_TMP/"* "$MODPATH/system/bin"
ln -sf "$MODPATH/system/bin/sec.velfoxruntime" "$MODPATH/system/bin/velfox_log"
rm -rf "$TMPDIR/libs"
...
if [ "$KSU" = "true" ] || [ "$APATCH" = "true" ]; then
	...
	BIN_PATH="/data/adb/modules/velfox/system/bin"
	for dir in $manager_paths; do
		[ -d "$dir" ] && {
			ui_print "- Creating symlink in $dir"
			ln -sf "$BIN_PATH/sec.velfoxruntime" "$dir/sec.velfoxruntime"
			ln -sf "$BIN_PATH/sec.velfoxruntime" "$dir/velfox_log"
			ln -sf "$BIN_PATH/velfox_profiler" "$dir/velfox_profiler"
			ln -sf "$BIN_PATH/velfox_utility" "$dir/velfox_utility"
		}
	done
fi
...
# Root Trace Cleanup
[ -d /data/velfox ] && rm -rf /data/velfox
[ -f /data/local/tmp/velfox_logo.png ] && rm -f /data/local/tmp/velfox_logo.png

# Configuration Setup
ui_print "- Velfox configuration setup"
...
9) ui_print "- Velfox still has many stories." ;;
esac
