...
if [ -n "$MMRL" ]; then
	...
	echo "- Please open the Velfox Tweaks WebUI via the module card."
	exit 0
fi
...
if [ -n "$MAGISKTMP" ]; then
	pm path io.github.a13e300.ksuwebui >/dev/null 2>&1 && {
		...
		am start -n "io.github.a13e300.ksuwebui/.WebUIActivity" -e id "velfox"
		exit 0
	}
	pm path com.dergoogler.mmrl.webuix >/dev/null 2>&1 && {
		echo "- Opening WebUI using WebUI X..."
		am start -n "com.dergoogler.mmrl.webuix/.ui.activity.webui.WebUIActivity" -e MOD_ID "velfox"
		exit 0
	}
fi
