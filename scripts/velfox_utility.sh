...
save_logs() {
	...
	log_file="/sdcard/Download/velfox_bugreport_$(date +"%Y-%m-%d_%H_%M").txt"
	SOC="Unknown"
...
Module Version: $(awk -F'=' '/version=/ {print $2}' /data/adb/modules/velfox/module.prop)
...
logcat() {
	...
	# Header
	echo -e "\e[1;36m┌────────────────────────────────────────────┐"
	echo -e "│      \e[1;37mVelfox Tweaks Log Viewer\e[1;36m          │"
	echo -e "└────────────────────────────────────────────┘\e[0m"

	# Info block
	echo -e "
\e[1;32mModule Version:\e[0m $(awk -F'=' '/version=/ {print $2}' /data/adb/modules/velfox/module.prop)
...
"
	...
	# Tail log
	tail -f $MODULE_CONFIG/velfox.log | while read -r line; do
		timestamp="${line:0:23}"
