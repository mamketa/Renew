...
void notify(const char* message) {
    ...
    if (exit != 0) [[clang::unlikely]] {
        log_velfox(LOG_ERROR, "Unable to post push notification, message: %s", message);
    }
}
...
void toast(const char* message) {
    ...
    if (ret != 0) [[clang::unlikely]] {
        log_velfox(LOG_WARN, "Unable to show toast message: %s", message);
    }
...
[[noreturn]] void sighandler(const int signal) {
    switch (signal) {
    case SIGTERM:
        log_velfox(LOG_INFO, "Received SIGTERM, exiting.");
        break;
    case SIGINT:
        log_velfox(LOG_INFO, "Received SIGINT, exiting.");
        break;
    }
 ...
 ***********************************************************************************/
void is_kanged(void) {
    if (systemv("grep -q '^name=Velfox Tweaks$' %s", MODULE_PROP) != 0) [[clang::unlikely]] {
        goto doorprize;
    }
...
doorprize:
    log_velfox(LOG_FATAL, "Module modified by 3rd party, exiting.");
