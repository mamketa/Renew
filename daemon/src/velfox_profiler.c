...
bool get_screenstate_normal(void) {
    static uint8_t fetch_failed = 0;
    char* screenstate = execute_command(
        "dumpsys power | grep -Eo 'mWakefulness=Awake|mWakefulness=Asleep' "
        "| awk -F'=' '{print $2}'"
    );

    if (screenstate) [[clang::likely]] {
        bool awake = IS_AWAKE(screenstate);
        free(screenstate);
        fetch_failed = 0;
        return awake;
    }
...
bool get_low_power_state_normal(void) {
    ...
    if (low_power) [[clang::likely]] {
        bool enabled = IS_LOW_POWER(low_power);
        free(low_power);
        fetch_failed = 0;
        return enabled;
    }
    fetch_failed++;
    log_velfox(LOG_ERROR,
               "Unable to fetch battery saver status");
    if (fetch_failed >= 6) {
        log_velfox(LOG_FATAL,
                   "get_low_power_state is out of order!");
